To-do list and new proposals:
 - [TODO] The map, the bar and the ball could be local variables. // DONE
 - [TODO] The bar should be generated always in the middle of the screen between matches. // DONE
 - [TODO] There must be a life-score in the screen. // DONE
 - [TODO] Same, but with the remaining destructible blocks. // PABLO(?)
 - [TODO] Generate quality doxygen docs for each function. 
 - [TODO] Extract bar and ball speeds to a game constant. // DONE
 - [TODO] Modularize even more the game.
 - [TODO] Check styles. // CHAT GPT
 - [TODO] Delete useless comments, and check for more unused variables.
 - [TODO] Cleaning directories deleting other projects, tasks...
 - [TODO] Delete unused peripherals.
 - [TODO] Add timer interrupts creating own sleep. // PABLO
 

 
 - [IDEA] The ball could be thrown with different angles. // IMPOSSIBLE
 - [IDEA] The buttons could have arrows. // DONE
 - [IDEA] Add a wait after a button is pressed in the 'check_button()' function. // DONE
 - [IDEA] Add animation when pressed button // DONE
